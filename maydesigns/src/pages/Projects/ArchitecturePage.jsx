...
const ArchitecturePage = () => {
  ...
  const architectureProjects = categoryProjects.architecture || [];

  useEffect(() => {
    fetchProjectsByCategory("architecture");
    fetchProjectStats();
  }, [fetchProjectsByCategory, fetchProjectStats]);
  ...
  const projectTypes = [
    { id: "all", name: "All Projects", count: architectureProjects.length },
    {
      id: "residential",
      name: "Residential",
      count: architectureProjects.filter(
        (p) => p.sub_category === "residential"
      ).length,
    },
    {
      id: "commercial",
      name: "Commercial",
      count: architectureProjects.filter((p) => p.sub_category === "commercial")
        .length,
    },
  ];

  const filteredProjects = architectureProjects.filter(
    (project) => filter === "all" || project.sub_category === filter
  );

  if (loading && architectureProjects.length === 0) {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden">
        <img
          src={project.images?.[0] || "/api/placeholder/600/400"}
          alt={project.name}
        ...
        <div className="absolute top-6 left-6">
          ...
          >
            {project.is_featured ? "Featured" : project.sub_category}
          </span>
        </div>
